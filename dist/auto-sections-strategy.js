...
const configSchema = z.strictObject({
    ...
    filter: z
        .object({
        include: z.array(filter$1).optional(),
        exclude: z.array(filter$1).optional(),
    })
        .optional(),
...
const filters = {
    ...
    domain: (_, value, { entity_id }) => value === E(entity_id),
    entity_id: (_, value, { entity_id }) => value === entity_id,
    hidden: (_, value, { hidden_by }) => value === (hidden_by !== null),
    state: ({ states }, value, { entity_id }) => value === states[entity_id].state,
};
function filter(hass, config, entity) {
...
function findEntity(allEntities, entityId) {
    return allEntities.find((obj) => obj.entity_id === entityId);
}
function findDevice(allDevices, deviceId) {
    return allDevices.find((obj) => obj.id === deviceId);
}
function findArea(allAreas, areaId) {
    return allAreas.find((obj) => obj.area_id === areaId);
}
function computeSectionTitle(sectionKey, config, context) {
    if (!config)
        return sectionKey;
    const [domain, rest] = config.split('.');
    const [key, field] = rest.split('|');
    const ctx = get$1(context, domain);
    const title = ctx.find((obj) => obj[key] === sectionKey)[field];
...
class AutoSectionsStrategy extends HTMLTemplateElement {
    static async generate(config, hass) {
        ...
        // @ts-expect-error
        const grouped = Object.groupBy(cards, (card) => {
            ...
            if (typeof group_by === 'string')
                return get$1(context, group_by);
            return group_by
                .map((option) => get$1(context, option))
...
customElements.define('ll-strategy-view-auto-sections-dev', AutoSectionsStrategy);
